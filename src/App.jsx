...
function App() {
  ...
  const handleAddOrUpdate = () => {
    if (isEdit) {
      ...
      setTasks([{ id: uuidv4(), task, isCompleted: false }, ...tasks]);
    }
    setTask("");
  };
  ...
  return (
    <>
      ...
      <div className="container mx-auto my-5 rounded-md p-3">
        ...
        <h2 className="text-xl textcolor font-bold mt-5">Tasks</h2>
        <ul className="list-none p-0 mt-3 flex flex-col gap-2">
          {tasks.length === 0 && (
            <p className="text-gray-600 text-center text-4xl opacity-45">
              {" "}
              No tasks available
            </p>
          )}
          {tasks.map((item) => (
            <li
              key={item.id}
              className={`${
                item.isCompleted ? "opacity-30" : "completed_bg"
              } flex items-center p-2 rounded-md transition duration-300`}
            >
              <input
                type="checkbox"
